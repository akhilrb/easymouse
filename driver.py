...
def startReceiver():
    ...
    configurePort()
    try:
        consolePrint("Trying "+ str(PORT)+" @ " + str(BAUDRATE)+" bps...\t"+ "Max Comm Speed: " + str("%.2f" %(float(BAUDRATE)/8096)) + " kBps", Priority.EVENT)
        sleep(1)
...
if serialReceiver.isOpen():
    while 1:
        try:
            ...
            try:
                consolePrint("Corrupt byte received",Priority.ERROR)
                serialReceiver =  serial.Serial(PORT, BAUDRATE)
                x = (serialReceiver.read()).decode("utf-8")
            except UnicodeDecodeError as ude:
            ...
            try:
                ...
                PORT = PORT_LIST[0]
                serialReceiver =  serial.Serial(PORT, BAUDRATE)
                consolePrint("Serial Stable", Priority.EVENT, GREEN)
                x = (serialReceiver.read()).decode("utf-8")
            except Exception as e:
